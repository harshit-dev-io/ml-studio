...
class Create_Org_API(APIView):
    authentication_classes = [Firebase_Authentication_Middleware]
    def post(self , request):
        serializer = Organization_Create_Serializer(data = request.data)
        if serializer.is_valid():
            success =  Create_Org(name = serializer.validated_data['name'] , user = request.user , type="team")
            if not success:
                return Response({"error" : "Error occurred while creating organization"} , status = status.HTTP_500_INTERNAL_SERVER_ERROR)
            return Response({"message" : "organization created successfully"} , status = status.HTTP_200_OK)
        else:
